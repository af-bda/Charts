...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    open override func initBuffers()
    {
        ...
        if _buffers.count != barData._dataSets.count
        {
            while _buffers.count < barData._dataSets.count
            {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for i in barData._dataSets.indices
        {
            guard let set = barData.dataSets[i] as? IBarChartDataSet else {
                    ...
                    for index in 0 ..< Int(ceil(Double(dataSet.entryCount) * animator.phaseX))
                    {
                        ...
                        else
                        {
                            ...
                            if let icon = e.icon, dataSet.isDrawIconsEnabled
                            {
                                ...
                                px += iconsOffset.x
                                py += iconsOffset.y
                                if let cgIcon = icon.cgImage {
                                    context.draw(cgIcon, in: CGRect(x: px, y: py, width: icon.size.width, height: icon.size.height))
                                }
                            }
                        }
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        {
            guard
                let set = barData.dataSets[high.dataSetIndex] as? IBarChartDataSet,
